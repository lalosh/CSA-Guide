...
customer Agreement:
    ...
    customer Agreement is the explicit definition of rules,resposiblities and
    execution of processes formally agreed upon
...
SLA:
    ...
    describe levels of Service using various attributes(availability,performance,...)
    and for each attribute SLA define `Thresholds` and `Financial Penalties associated with
    violation of these Thresholds`
    Note: there is usually a mismatch between metrics measured by the provider and those
    measured by the customer,specially in SaaS because application level services
    are impacted by many factors. This is why CSA for SaaS lack tight service level guarantees
...
2-Evaluate Data and Business level policies
    ...
    *Data policies:
        ...
        #Change Management and Notifications: Is there a change later on some policy?
            if yes how much time is given for customers to get prepared
            before a change is done?
    *Business policies:
        #Guarantees: Services Attributes. 
                    example: service availability is 99.9%
        #AUP: answers: how the customer may use the service? what actions the provider
            may take in event of an illegal use of the service?
...
3-Sevice level objectives:
    ...
    general service level objectives(should be taken via clear and consistent measurements):
        ...
        *downtime
        *response time: 
            elapsed time from when a service is invoked to when it is completed
            (usually in milliseconds)
        *data persistence
        *portability: 
            store data in standard formats to make it portable in case a customer
            changed his cloud service provider
        *scalability
        *system qualities:
            reliability
            fault tolerance
...
4-security and privacy to data and application
    ...
    *security considerations:
        ...
        #Restrictions:
            ...
            2-monitor for large internal data migration with
                `DAM`(DB activity monitoring) and `FAM`(file activity monitoring)
        ...
        #Notifications:
            the provider should notify the customer of the occurence of any breach
            of its system
        
        #Privacy:
            Give the "right to be forgotten" to customers which enable them
            to delete their data and backups
...
7-The EXIT process
    ...
    *portability:
        data maintained on the provider's cloud resources should be stored using
        standard formats to ensure data portability
